...
d3.csv("https://gist.githubusercontent.com/soybean/1638b5bb7ec5edb8c9ce8e3d5635345e/raw/8f4171d72cfafd227958a148def9ef801a3a250e/birdmap.csv", function(data) {
  ...
  d3.json('https://gist.githubusercontent.com/soybean/b0def4d8a0bb267dd3e7fb12387b1e2b/raw/881b01cb5911010a59d8bd47068a93130f724431/gistfile1.txt', function(json) {
	  ...
	  var concernScore = data[i].concernScore;
		  ...
		  json.features[i].properties.concernScore = concernScore;
	          json.features[i].properties.scientific = scientific;	
                  json.features[i].properties.audio = audio;
                  break;
              }
          }
      }
      ...
      for (var i=0; i < 6; i++) {
        ...
        var newState = json.features[5];
        // make sure the same two states aren't quizzed twice
        /*
        while(quizzedStates.indexOf(newState) != -1) {
            currentState = Math.floor(Math.random()*50)
            newState = json.features[currentState];
        }
        */
        quizzedStates.push(newState);
        ...
        var y = actual.selectAll('path')
                    ...
                    .style('stroke', '#fff')
                    .style("fill", getColor);
        var newcentroid = path.centroid(newState);
        var newwidth = y.node().getBBox().width;
        var stateScale = 200/width;

        y.attr('transform', "translate(100,150)scale(1.5)translate("+-newcentroid[0]+","+-newcentroid[1]+")");
        for (var j = 0; j<3; j++) {
        ...
        var newNewState = json.features[5];
        ...
        var o = actual.selectAll('path')
                    ...
                    .style('stroke', '#fff')
                    .style("fill", getColor);
          var little = actual.selectAll('path')
                        .append('rect')
                        .attr("x", 10)
                        .attr("y", 10)
                        .attr("width", 50)
                        .attr("height", 50);
          var newishwidth = o.node().getBBox().width;
          var stateScale = 200/width;
          console.log(width*stateScale);
          console.log(width);
...
function generateHTML(d) {
    ...
    var audio = d.properties.audio;
    return "<div class='box-title'>" + stateName + "<div id='bird-name'>" + birdName + "</div><div id='scientific-name'>"+scientificName + "</div><br><audio controls loop> <source src='audio/" + audio + "' type 'audio/mpeg'></audio></div>";
}
